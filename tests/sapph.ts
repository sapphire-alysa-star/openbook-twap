...
describe("openbook-twap", () => {
    ...
    it("Is initialized!", async () => {
        ...
        for (let i = 0; i < 5; i++) {
            let openBuyOrders = await openbook.createOpenOrders(
                market,
                new BN(i + 1),
                `buyOrders${i}`
            );
            buyOrders.push(openBuyOrders);
            console.log(`Created buyOrders${i}`);
            await openbook.deposit(
                buyOrders[i],
                await openbook.getOpenOrders(buyOrders[i]),
                storedMarket,
                metaAccount,
                usdcAccount,
                new BN(META_UNIT),
            ...
            console.log(`Deposited to buyOrders${i}`);
        }
        ...
        for (let k = 0; k < 5; k++) {
            ...
            let openSellOrders = await openbook.createOpenOrders(
                market,
                new BN(k + 6),
                `sellOrders${k}`
            );

            sellOrders.push(openSellOrders);
            console.log(`Created sellOrders${k}`);

            await openbook.deposit(
                sellOrders[k],
                await openbook.getOpenOrders(sellOrders[k]),
                storedMarket,
                metaAccount,
                usdcAccount,
                new BN(META_UNIT),
            ...
            console.log(`Deposited to sellOrders${k}`);
